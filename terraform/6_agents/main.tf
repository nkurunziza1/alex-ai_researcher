...
locals {
  lambda_packages_bucket_name = "alex-lambda-packages-${data.aws_caller_identity.current.account_id}"
}
...
resource "aws_iam_role" "lambda_agents_role" {
  count = var.reuse_existing_agent_infrastructure ? 0 : 1
...
data "aws_iam_role" "lambda_agents_existing" {
  count = var.reuse_existing_agent_infrastructure ? 1 : 0
  name  = "alex-lambda-agents-role"
}

locals {
  lambda_agents_role_arn  = var.reuse_existing_agent_infrastructure ? data.aws_iam_role.lambda_agents_existing[0].arn : aws_iam_role.lambda_agents_role[0].arn
  lambda_agents_role_id   = var.reuse_existing_agent_infrastructure ? data.aws_iam_role.lambda_agents_existing[0].id : aws_iam_role.lambda_agents_role[0].id
  lambda_agents_role_name = var.reuse_existing_agent_infrastructure ? data.aws_iam_role.lambda_agents_existing[0].name : aws_iam_role.lambda_agents_role[0].name
}

# IAM policy for Lambda agents
resource "aws_iam_role_policy" "lambda_agents_policy" {
  name = "alex-lambda-agents-policy"
  role = local.lambda_agents_role_id
...
resource "aws_iam_role_policy_attachment" "lambda_agents_basic" {
  role       = local.lambda_agents_role_name
...
resource "aws_s3_bucket" "lambda_packages" {
  count  = var.reuse_existing_agent_infrastructure ? 0 : 1
  bucket = local.lambda_packages_bucket_name
...
data "aws_s3_bucket" "lambda_packages_existing" {
  count  = var.reuse_existing_agent_infrastructure ? 1 : 0
  bucket = local.lambda_packages_bucket_name
}

locals {
  lambda_packages_bucket_id = var.reuse_existing_agent_infrastructure ? data.aws_s3_bucket.lambda_packages_existing[0].id : aws_s3_bucket.lambda_packages[0].id
}
...
resource "aws_s3_object" "lambda_packages" {
  ...
  bucket = local.lambda_packages_bucket_id
...
resource "aws_lambda_function" "planner" {
  ...
  role          = local.lambda_agents_role_arn

  # Using S3 for deployment package (>50MB)
  s3_bucket        = local.lambda_packages_bucket_id
...
# Log groups: created automatically when each Lambda runs. Omit Terraform-managed groups to avoid
# ResourceAlreadyExists when Lambda (or a prior deploy) already created /aws/lambda/alex-*.