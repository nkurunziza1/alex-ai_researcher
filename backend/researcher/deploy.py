...
def run_command(cmd, capture_output=False, shell=False, cwd=None):
    """Run a command and handle errors."""
    try:
        result = subprocess.run(
            cmd, shell=shell, capture_output=capture_output, text=True, check=True, cwd=cwd
        )
        if capture_output:
...
def main():
    ...
    # Build Docker image (context = backend/ so researcher/ + agent_llm/ are available)
    researcher_dir = Path(__file__).parent.resolve()
    backend_dir = researcher_dir.parent
