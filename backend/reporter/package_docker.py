    ...
    with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as temp_dir:
        ...
        docker_cmd = [
            ...
            *_docker_run_user_args(),
            ...
            "-v",
            f"{backend_dir}/agent_llm:/agent_llm",
            "--entrypoint",
            "/bin/bash",
            "public.ecr.aws/lambda/python:3.12",
            "-c",
            """cd /build && pip install --target ./package -r requirements.txt && pip install --target ./package --no-deps /database && pip install --target ./package --no-deps /agent_llm""",
        ]
