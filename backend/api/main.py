...
# Local dev: when SQS is unset, spawn planner subprocess if true (see Guide 6 / deploy)
RUN_PLANNER_LOCALLY = os.getenv("RUN_PLANNER_LOCALLY", "false").lower() in ("1", "true", "yes")


def _spawn_planner_subprocess(job_id: str) -> None:
    """Run planner in a separate process so /api/analyze returns immediately."""
    planner_dir = Path(__file__).resolve().parent.parent / "planner"
    script = planner_dir / "run_planner_job.py"
    if not script.is_file():
        logger.error("Local planner script missing: %s", script)
        return
    try:
        log_path = planner_dir / "planner_local.log"
        log_f = open(log_path, "a", encoding="utf-8")
        subprocess.Popen(
            ["uv", "run", str(script), job_id],
            cwd=str(planner_dir),
            start_new_session=True,
            stdout=log_f,
            stderr=subprocess.STDOUT,
        )
        logger.info("Spawned local planner subprocess for job %s (log: %s)", job_id, log_path)
    except OSError as e:
        logger.error("Could not start local planner: %s", e)
...
async def get_clerk_credentials(
    creds: HTTPAuthorizationCredentials = Depends(clerk_guard),
) -> HTTPAuthorizationCredentials:
    """Validate Clerk JWT once per request; reuse for both user id and token claims."""
    return creds


async def get_current_user_id(
    creds: HTTPAuthorizationCredentials = Depends(get_clerk_credentials),
) -> str:
...
async def trigger_analysis(request: AnalyzeRequest, clerk_user_id: str = Depends(get_current_user_id)):
    ...
    try:
        ...
        elif RUN_PLANNER_LOCALLY:
            _spawn_planner_subprocess(str(job_id))
            logger.info("SQS_QUEUE_URL not set; started local planner subprocess for job %s", job_id)
        else:
            logger.warning(
                "SQS_QUEUE_URL not configured — job %s will stay pending until you set SQS_QUEUE_URL "
                "or RUN_PLANNER_LOCALLY=true in projects/alex/.env",
                job_id,
            )
