...
from alex_agent_llm import create_openrouter_model
...
def load_portfolio_summary(job_id: str, db) -> Dict[str, Any]:
    ...
    try:
        ...
        _yur = user.get("years_until_retirement")
        _tri = user.get("target_retirement_income")
        # DB may store NULL: dict.get(key, default) still returns None when the key exists.
        years_until_retirement = 30 if _yur is None else _yur
        target_retirement_income = float(_tri if _tri is not None else 80000)
